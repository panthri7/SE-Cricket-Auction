...
# Theme accents
ACCENT_1 = "#b28dff"   # light purple
ACCENT_2 = "#34d0c3"   # sea green
DARK_PURPLE = "#4b0082"
TEAM_BTN_BG = "#e9ddff"   # light-purple background for team buttons
TEAM_BTN_TXT = "#2b195e"
TEAM_BTN_BORDER = "#c9b8ff"
...
st.markdown(
    ...
    <style>
      ...
      .title-bar h2 {{
        margin: 0;
        color: {DARK_PURPLE};   /* (1) Dark purple title text */
        font-weight: 800;
      }}
      .bigcard {{
      ...
      /* (4) Light purple background for team buttons */
      .team-bid button {{
        background: {TEAM_BTN_BG} !important;
        color: {TEAM_BTN_TXT} !important;
        border: 1px solid {TEAM_BTN_BORDER} !important;
      }}
      /* Make the little pill buttons a bit roomier */
      .stButton>button {{
...
with tabs[0]:
    ...
    if st.session_state.current_index >= len(players_df):
        ...
        # (4 & 5) Team bid buttons with light-purple bg and info (players left • budget)
        bid_cols = st.columns(3)
        for i, team in enumerate(teams):
            left = players_left(team)
            budget = st.session_state.budgets.get(team, 0)
            label = f"{team}  ({left} left • {st.session_state.currency_symbol}{budget})"
            with bid_cols[i % 3]:
                st.markdown('<div class="team-bid">', unsafe_allow_html=True)
                st.button(label, key=f"bid_{i}", on_click=place_bid, args=(team,))
